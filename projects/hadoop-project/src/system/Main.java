		...
		if(args[0].equals("-m") && args.length == 4) {
			...
			if(addr.length == 2) {
				try {
					...
					System.out.println("ERROR: To launch a slave, run './mapReduce -s <hostname>:<slave port number> <working directory>',\n" +
				...
				System.out.println("ERROR: To launch a slave, run './mapReduce -s <hostname>:<slave port number> <working directory>',\n" +
						"invalid <hostname>:<slave port number>, make sure to use ':'.");
				return;
			}
			...
			if(addr.length == 2) {
				try {
					...
					System.out.println("ERROR: To launch a slave, run './mapReduce -s <hostname>:<client port number> <working directory>',\n" +
				...
				System.out.println("ERROR: To launch a slave, run './mapReduce -c <hostname>:<client port number> <working directory>',\n" +
						"invalid <hostname>:<client port number>, make sure to use ':'.");
				return;
			}
		}
